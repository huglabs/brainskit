...
|---|---|---|---|
| `validation_error` | 2 | The request is malformed | Change the request |
| `conflict` | 2 | The request was fine; the vault moved under it | Re-read state, rebuild the same request, retry |
| `not_configured` | 2 | This installation cannot serve it — no model mapped, optional dependency absent, variable unset, integration off | Stop retrying; report what the operator must configure |

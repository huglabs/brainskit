...
# payload on stdin and asks the brainkit CLI whether a direct write to that
...
if [ -z "$REASON" ]; then
    REASON='This path is written by the brainkit apply gate, not by a file-write tool.'
fi

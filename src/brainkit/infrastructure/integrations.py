...
class NativeIntegrations:
    ...
    def _sync_neo4j(
        ...
        try:
            ...
            raise ValidationError(
                ...
                details={"hint": "Install brainkit[neo4j]"},
            ) from exc
    ...
    def _sync_postgres(
        ...
        try:
            ...
            raise ValidationError(
                ...
                details={"hint": "Install brainkit[postgres]"},
            ) from exc
    ...
    def _database_up(
        ...
        if not shutil.which("docker"):
            raise ValidationError("Managed integrations require Docker")
        container = _container_name(self.vault.root, name, policy.options)
...
def _vendor_boundary(
    ...
    *,
    ...
    """Translate vendor driver failures into the brainkit error contract."""
    try:
        yield
    except BrainkitError:
        raise
...
def _docker(
    ...
    try:
        ...
        raise ValidationError("Docker command failed", details={"reason": str(exc)}) from exc
...
def _probe_web_identity(host: str, port: int) -> str | None:
    ...
    try:
        ...
        return None
    if not isinstance(payload, dict) or payload.get("service") != "brainkit-web":
        return None

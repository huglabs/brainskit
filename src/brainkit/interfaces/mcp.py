...
def run_stdio(service: BrainkitService) -> None:
...
def run_http(
    ...
    *,
    ...
    server = BrainkitMcpHttpServer((host, port), BrainkitMcpHttpHandler)
...
class BrainkitMcpHttpServer(ThreadingHTTPServer):
    daemon_threads = True
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if not self._authorized():
            self._send_http_error(
                ...
                "unauthorized",
                authenticate='Bearer realm="brainkit-mcp"',
            )
            return
        ...
        try:
            ...
            # A body brainkit cannot even parse is a malformed HTTP request,
...
def _report_internal_error(context: str, exc: BaseException) -> None:
    ...
    print(f"brainkit mcp: unhandled error during {context}", file=sys.stderr)
    traceback.print_exception(exc, file=sys.stderr)
    sys.stderr.flush()


def _handle(
    service: BrainkitService, request: dict[str, Any]
) -> dict[str, Any] | None:
    ...
    if method == "initialize":
        ...
        return _result(
            request_id,
            {
                ...
                "serverInfo": {"name": "brainkit", "version": __version__},
            },
        )
    if method == "ping":
...
def _call_tool(
    service: BrainkitService, name: str, arguments: dict[str, Any]
) -> dict[str, Any]:

...
from brainkit.application.codegraph import CODE_PROJECTION, CodeGraph
...
from brainkit.application.privacy import _context_branches
...
class Health:
    ...
    def lint(self, *, semantic: bool = False) -> dict[str, Any]:
        ...
        if semantic:
            context = self.retrieval.context("contradictions unsupported claims", limit=20)
    ...
    def _enforcement_state(self) -> dict[str, Any]:
        ...
        layers = [
            ...
            ),
            {
                ...
                "detail": "active"
                if commit_active
                else f"{root} is not a git repository or has no brainkit pre-commit hook",
            },
            {
                "layer": "instructions",
    ...
    def _agent_workspace(self, agent: str = "claude") -> Path:
        ...
        Falls back to the vault, which is both the historical behaviour and the
        right answer for a standalone vault. An adapter written before the
        workspace was recorded therefore keeps reporting exactly as it did.
        """

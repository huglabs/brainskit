...
async function load(){try{let [status,proposals]=await Promise.all([api('/api/status'),api('/api/proposals?status=pending')]);renderStatus(status);renderProposals(proposals);document.getElementById('health').textContent=status.healthy?'healthy':'needs attention'}catch(error){document.getElementById('health').textContent=error.message}if(!state.nodes.length)loadGraph(state.source)}

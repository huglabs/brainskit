...
#: Markers delimiting the managed block in an agent's instruction file.
#
# Owned here rather than in `interfaces/cli.py`, which writes the block, because
# `application/health.py` reads it to decide whether the instructions layer is
# live. The layering rule -- application must not import interfaces -- is right,
# and the previous response to it was to copy the string into both. Writer and
# readers could then disagree in silence about what "installed" means, and per
# this repository's own history that divergence has already shipped twice.
INSTRUCTION_START = "<!-- brainskit:start -->"
INSTRUCTION_END = "<!-- brainskit:end -->"

#: Marks a file as generated by `bk hooks install` rather than hand-written.

...
from brainskit.infrastructure.integrations import vault_id
...
class VaultRegistry:
    ...
    def _write(self, entries: Sequence[RegisteredVault]) -> None:
        ...
        # mode exists to prevent, and this is brainskit's own directory.
...
def default_registry_path(environment: Mapping[str, str] | None = None) -> Path:
    ...
    # old file is honoured until something writes the new one -- the
    # alternative is a registry that silently reads as empty after an upgrade.
    if not current.exists():
        legacy = base / "brainkit" / "vaults.json"
        if legacy.exists():
            return legacy
    return current
